...
const shopsLocationInfo = document.getElementById('shopsLocationInfo');
const shopsProximityTitle = document.getElementById('shopsProximityTitle');
...
const shopsDButton = document.getElementById('shopsDButton');
...
function closeMenu() {
    if (isLocked) {
        console.log('Menu verrouillé, fermeture ignorée');
        return;
    }
    ...
    mainButton.classList.remove('expanded-shops');
    
    // CORRECTION : Supprimer le transform qui fait rétrécir
    mainButton.style.zIndex = '100';
    subButtons.style.zIndex = '90';
    
    // S'assurer que le bouton reprend sa position normale
    mainButton.style.transform = '';
    mainButton.style.width = '';
    ...
    shopsLocationInfo.style.display = 'none';
    shopsProximityTitle.style.display = 'none';
    distanceWarning.style.display = 'none';
    
    // CORRECTION : NE PAS réinitialiser l'opacité et transform ici
    // Laisser le CSS gérer l'animation via la classe .visible
    ...
    // SUPPRIMER complètement ce setTimeout qui cause le rétrécissement
    // setTimeout(() => {
    //     mainButton.style.zIndex = '100';
    //     subButtons.style.zIndex = '90';
    //     subButtons.style.opacity = '0';
    //     subButtons.style.transform = 'scale(0.8)';  // ⛔ CAUSE LE RÉTRÉCISSEMENT
    // }, 50);
}

function openMenu() {
    // CORRECTION : S'assurer que tout est réinitialisé avant d'ouvrir
    mainButton.style.zIndex = '1100';
    subButtons.style.zIndex = '1101';
...
function applyLockedAppearance(container) {
    ...
    if (dButton) {
        dButton.style.backgroundColor = 'rgba(78, 205, 196, 0.3)';
        dButton.style.color = '#121212';
        dButton.style.border = '2px solid rgba(78, 205, 196, 0.8)';
        dButton.style.transform = 'scale(1.1)';
    }
}
...
function resetButtonAppearance(container) {
    ...
    if (dButton) {
        ...
        dButton.style.transform = '';
    }
}

// Écouteurs d'événements pour les boutons D
...
shopsDButton.addEventListener('click', function(event) {
    event.stopPropagation();
    toggleLock(shopsContainer);
});
...
// Clic sur le pied pour les magasins
const shopsButton = subButtons.querySelector('.sub-button:nth-child(5)');
shopsButton.addEventListener('click', async function(event) {
    event.stopPropagation();
    mainButton.classList.add('expanded-shops');
    mainButton.classList.remove('expanded-burger');
    mainButton.classList.remove('expanded-trophy');
    mainButton.classList.remove('expanded-news');
    ...
    subButtons.style.display = 'none';
    shopsContainer.style.display = 'flex';
    restaurantContainer.style.display = 'none';
    transportContainer.style.display = 'none';
    f1Container.style.display = 'none';
    trophyContainer.style.display = 'none';
    ...
    shopsProximityTitle.style.display = 'none';
    shopsLocationInfo.style.display = 'none';
    showShopsLoadingAnimation();
    
    await getUserLocation();
    shopsLocationInfo.style.display = 'block';
    shopsLocationInfo.textContent = `📍 Recherche dans ${userCity || 'votre zone'}`;
...
async function findNearbyShops() {
    ...
    try {
        ...
        const shops = await findShopsWithOpenStreetMap(lat, lon);
        
        if (shops.elements && shops.elements.length > 0) {
            const shopsWithDistance = shops.elements.map(shop => {
                const distance = calculateDistance(userLocation.lat, userLocation.lon, shop.lat, shop.lon);
                return { ...shop, distance: distance };
            }).sort((a, b) => a.distance - b.distance).slice(0, 4);

            // Afficher IMMÉDIATEMENT sans attendre les statuts
            displayRealShops(shopsWithDistance);
            
            // Mettre à jour les statuts après affichage
            updateShopsWithRealStatus(shopsWithDistance);
            
        } else {
            displayFallbackShops();
        }

    } catch (error) {
        console.error('Erreur recherche magasins:', error);
        displayFallbackShops();
    }
}

async function updateShopsWithRealStatus(shops) {
    // Mettre à jour les statuts en arrière-plan
    for (let i = 0; i < shops.length; i++) {
        try {
            const openingStatus = await getRealOpeningStatus(shops[i]);
            if (openingStatus) {
                updateShopCardStatus(i, openingStatus);
            }
        } catch (error) {
...
function updateShopCardStatus(index, openingStatus) {
    const shopCards = shopsGrid.querySelectorAll('.shop-card');
    if (shopCards[index]) {
        const statusElement = shopCards[index].querySelector('.opening-status');
        const hoursElement = shopCards[index].querySelector('.shop-hours');
        ...
        if (statusElement && hoursElement) {
            ...
            if (openingStatus.isOpen && openingStatus.closingTime) {
                ...
                hoursElement.textContent = 'Heures disponibles';
            }
        }
    }
}

async function findShopsWithOpenStreetMap(lat, lon) {
    try {
        const radius = 500;
        const query = `
            [out:json][timeout:15];
...
async function getRealOpeningStatus(shop) {
    try {
        // Méthode RAPIDE: Heures directement dans les tags OSM
        const openingHours = shop.tags?.opening_hours;
        
        if (openingHours) {
            return parseOSMOpeningHours(openingHours);
        }
        
        return null; // ❌ Pas d'heures disponibles
...
function parseOSMOpeningHours(hoursString) {
    try {
        ...
        // Convertir le jour en format OSM
        const daysOSM = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
        const currentDayOSM = daysOSM[currentDay];
        
        // Nettoyer la string
        const cleanHours = hoursString.replace(/"/g, '').trim();
        ...
        for (const rule of rules) {
            ...
            // Vérifier si fermé
            if (trimmedRule.toLowerCase().includes('off') || 
                trimmedRule.toLowerCase().includes('closed')) {
                return { isOpen: false, closingTime: null };
            }
            
            // Vérifier si la règle s'applique aujourd'hui
            if (isRuleForToday(trimmedRule, currentDayOSM)) {
                const timeMatch = trimmedRule.match(/(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})/);
                if (timeMatch) {
                    const openHour = parseInt(timeMatch[1]) * 100 + parseInt(timeMatch[2]);
                    const closeHour = parseInt(timeMatch[3]) * 100 + parseInt(timeMatch[4]);
                    
                    const isOpen = currentTime >= openHour && currentTime <= closeHour;
                    const closingTime = `${timeMatch[3]}:${timeMatch[4]}`;
...
function isRuleForToday(rule, currentDay) {
    try {
        // Cas 1: Jour spécifique
        if (rule.includes(currentDay)) {
            return true;
        }
        
        // Cas 2: Plage de jours
        const rangeMatch = rule.match(/([A-Za-z]{2})-([A-Za-z]{2})/);
        if (rangeMatch) {
...
function displayRealShops(shops) {
    shopsProximityTitle.style.display = 'block';
    shopsLocationInfo.style.display = 'block';
    shopsLocationInfo.textContent = `📍 ${shops.length} magasins près de vous`;
    
    const fragment = document.createDocumentFragment();
    
    shops.forEach((shop, index) => {
        const shopCard = createShopCard(shop, index);
        fragment.appendChild(shopCard);
    });
...
function createShopCard(shop, index) {
    ...
    nameElement.textContent = shop.tags?.name || 'Magasin';
    
    const addressElement = document.createElement('div');
    addressElement.className = 'shop-address';
    addressElement.textContent = getCompleteAddress(shop);
    
    header.appendChild(nameElement);
    header.appendChild(addressElement);
    shopCard.appendChild(header);
    
    // Statut d'ouverture - TOUJOURS ❌ au début
    const hoursInfo = document.createElement('div');
    hoursInfo.className = 'shop-info-row';
    
    const statusElement = document.createElement('div');
    statusElement.className = 'opening-status status-unknown';
    statusElement.innerHTML = '❌ Statut inconnu';
    
    const hoursElement = document.createElement('div');
    hoursElement.className = 'shop-hours';
    hoursElement.innerHTML = '❌ Heures non disponibles';
    
    hoursInfo.appendChild(statusElement);
    hoursInfo.appendChild(hoursElement);
    shopCard.appendChild(hoursInfo);
    
    // Type et distance
    const typeInfo = document.createElement('div');
    typeInfo.className = 'shop-info-row';
    
    const typeElement = document.createElement('div');
    typeElement.className = 'shop-type';
    typeElement.textContent = getShopType(shop);
    
    const distanceElement = document.createElement('div');
    distanceElement.className = 'bus-distance';
    const distanceMeters = Math.round(shop.distance * 1000);
    distanceElement.textContent = `${distanceMeters}m`;
    
    typeInfo.appendChild(typeElement);
    typeInfo.appendChild(distanceElement);
    shopCard.appendChild(typeInfo);
    
    // Affluence - TOUJOURS ❌
    const crowdInfo = document.createElement('div');
    crowdInfo.className = 'shop-info-row';
    
    const crowdLabel = document.createElement('div');
    crowdLabel.textContent = 'Affluence:';
    
    const crowdElement = document.createElement('div');
    crowdElement.className = 'shop-crowd';
    crowdElement.innerHTML = '❌';
    
    crowdInfo.appendChild(crowdLabel);
    crowdInfo.appendChild(crowdElement);
    shopCard.appendChild(crowdInfo);
...
function getShopType(shop) {
    ...
    const types = {
        ...
        'hardware': '🛠️ Quincaillerie',
        'books': '📚 Librairie',
        'florist': '💐 Fleuriste',
        'jewelry': '💎 Bijouterie'
    };
    
    return types[shopType] || '🏬 Magasin';
}

function showShopsLoadingAnimation() {
    shopsGrid.innerHTML = `
...
function displayFallbackShops() {
    shopsGrid.innerHTML = `
        <div style="text-align: center; padding: 20px; color: #aaa; grid-column: 1 / -1;">
            <div style="font-size: 24px; margin-bottom: 10px;">🏬</div>
            <div>Aucun magasin trouvé à proximité</div>
            <div style="font-size: 10px; margin-top: 10px; color: #666;">Réessayez dans une zone plus commerçante</div>
        </div>
    `;
}
...
document.addEventListener('click', function() {
    if (isExpanded || mainButton.classList.contains('expanded-burger') || 
        mainButton.classList.contains('expanded-trophy') || mainButton.classList.contains('expanded-news') ||
        mainButton.classList.contains('expanded-shops')) {
        closeMenu();
    }
});
